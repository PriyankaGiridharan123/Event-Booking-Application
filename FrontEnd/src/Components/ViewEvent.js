import React, { useEffect, useState } from 'react'
import { Link, useNavigate } from 'react-router-dom'
import EventService from '../Service/EventService';
import { Pagination } from '@mui/lab';
import axios from 'axios';
import './ViewEvent.css'
const ViewEvent=()=> {
  const nav=useNavigate();
  const [currentPage, setCurrentPage] = useState(1);
  const [totalPages, setTotalPages] = useState(0);
  const itemsPerPage = 3;
  const [sortBy, setSortBy] = useState('id');
  const [sortOrder, setSortOrder] = useState('asc');
  const [events, setEvents] = useState([])

  useEffect(() => {
      getAllEvents();
  }, [currentPage,sortBy,sortOrder])

  const token=localStorage.getItem("jwtToken")
  if(token===null){
    nav("/")
   
  }
  const [user, setuser] = useState([]);
  useEffect(() => {
    const storeduser = JSON.parse(localStorage.getItem('userData')) || [];
    setuser(storeduser);
  }, []);


  const getAllEvents = () => {
      axios.get(`http://localhost:8080/event/${currentPage}/${itemsPerPage}/${sortBy}/${sortOrder}`).then((response) => {
      const{content,totalPages}=response.data;   
      setEvents(response.data)
      setTotalPages(totalPages);
          console.log(response.data);
      }).catch(error =>{
          console.log(error);
      })
  }
  const handleSort = (column) => {
    if (column === sortBy) {
      setSortOrder(sortOrder === 'asc' ? 'desc' : 'asc');
    } else {
      setSortBy(column);
      setSortOrder('asc');
    }
  };

  const deleteEvent = (eId) => {
    if(window.confirm("Sure to Delete?")){
     EventService.deleteEvent(eId).then((response) =>{
      getAllEvents();

     }).catch(error =>{
         console.log(error);
     })
    }
  }
  const deleteAllEvent = () => {
    if(window.confirm("Sure to Delete All Events?")){
     EventService.deleteAllEvent().then((response) =>{
      getAllEvents();

     }).catch(error =>{
         console.log(error);
     })
    }
  }
  const LogoutHandler=()=>{
    if(window.confirm("Sure to Logout?")){
      nav("/")
    }
  }
  const handlePageChange = (event, page) => {
    setCurrentPage(page);
  };

  return (
    <div id="container">
    <div className='view-event-container'>
            <div id="h" ><h1>UNPLUGGED EVENTS</h1></div>
    <table id="table">
              <thead>
                <tr>
                  <th onClick={() => handleSort('id')}>Id {sortBy === 'id' && (sortOrder === 'asc' ? '▲' : '▼') }</th>
                  <th>Event</th>
                  <th>Price</th>
                  <th  onClick={() => handleSort('date')}>Date {sortBy === 'date' && (sortOrder === 'asc' ? '▲' : '▼') }</th>
                  <th>Venue & Time</th>
                  <th>Available Ticket</th>
                  <th>Actions</th>
                </tr>
              </thead>
              <tbody>
              {
                events.content&&events.content.map(
                    (eventt) => (
      <tr key={eventt.id}>
             <th id="id"> {eventt.id}</th>
             <th id="event"> {eventt.event}</th>
             <th id="price"> {eventt.price}</th>
             <th id="date">{eventt.date}</th>
             <th id="venue"> {eventt.venue}</th>
             <th id="available"> {eventt.available}</th>
             <th id="action"><Link  to={`/update/${eventt.id}`}>
             <button id="actions">
             <img src="data:image/png;base64," width="80%" height="100%"></img></button></Link> 
             <button id="action-del"  onClick = {() => deleteEvent(eventt.id)}>
       <img src="https://www.freeiconspng.com/thumbs/cross-png/red-cross-png-33.png" width="90%" height="110%"></img></button></th>
            
      </tr>
    ))}
              </tbody>
            </table>
            <Pagination className='page'
            count={totalPages}
            page={currentPage}
            onChange={handlePageChange}
          />
    <button id="logout" onClick={LogoutHandler}>Logout</button>
    <button id="delbtn" onClick={deleteAllEvent} >Delete Event</button>
    </div>
    </div>
  )
}

export default ViewEvent;